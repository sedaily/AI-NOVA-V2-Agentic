...
const MainContent = ({
  ...
  const handleTitlesGenerated = (data) => {
    console.log('📤 handleTitlesGenerated 호출:', data);
    
    if (data.type === 'agent_streaming') {
      // Agent 스트리밍 - ChatPage로 이동
      console.log('🚀 Agent 스트리밍 모드:', { message: data.message, agentType: data.agentType });
      onStartChat(data.message, null, {
