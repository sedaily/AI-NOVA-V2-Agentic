...
class WebSocketService {
      ...
      try {
        ...
        this.ws.onmessage = (event) => {
          try {
            ...
            // console.log("웹소켓 메시지 수신:", data); // 로그 비활성화
  ...
  sendMessage(message, engineType = "11", conversationId = null, conversationHistory = null, idempotencyKey = null, selectedModel = 'claude-opus-4-5-20251101', webSearchEnabled = false, buddyService = null) {
    // 실제 WebSocket으로 메시지 전송
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      // CMS 사용자 식별을 위한 userId 설정
      const userInfo = JSON.parse(localStorage.getItem('userInfo') || '{}');
      const userId = userInfo.userId || userInfo.email || userInfo.username || import.meta.env.VITE_CMS_USER_ID || 'anonymous';
      ...
      // 버디 서비스 정보 추가
      if (buddyService) {
        payload.promptTable = buddyService.promptTable;
        payload.promptType = buddyService.promptType;
        payload.serviceCode = buddyService.serviceCode;
        payload.buddyType = localStorage.getItem('currentBuddyType') || null;
      }
  ...
  async simulateAIResponse(message, engineType, selectedModel = 'claude-opus-4-5-20251101') {
    ...
    console.log('🔍 API 모드 확인:', { useMockAPI, hasClaudeKey, VITE_USE_MOCK_API: import.meta.env.VITE_USE_MOCK_API });

    if (!useMockAPI && hasClaudeKey) {
      await this.callClaudeAPI(message, engineType, selectedModel);
    } else {
      console.log('⚠️ 모의 응답 사용 중');
  ...
  async callClaudeAPI(message, engineType, selectedModel) {
    try {
      ...
      this.messageHandlers.forEach((handler) => {
        handler({ type: "error", message: `오류가 발생했습니다: ${error.message}` });
      });
    }
  }
