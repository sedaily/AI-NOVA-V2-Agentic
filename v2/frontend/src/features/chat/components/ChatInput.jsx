  ...
  (
    ...
    const handleAgentButton = (agentType) => {
      ...
      if (selectedAgentType === agentType) {
        ...
        toast.success('일반 모드로 전환', {
          duration: 1500,
          position: 'top-center',
        });
      } else {
        console.log('✅ 모드 선택:', agentType);
        setSelectedAgentType(agentType);
        console.log('📊 setSelectedAgentType 호출 완료');
        toast.success(`${agentType} 모드 선택됨`, {
          duration: 1500,
          position: 'top-center',
        });
      }
      ...
      if ((message.trim() || uploadedFiles.length > 0) && !isLoading) {
        ...
        if (selectedAgentType && messageText) {
          ...
          try {
            ...
            console.error('❌ AI Agent 오류 상세:', {
              message: error.message,
              stack: error.stack,
              name: error.name
            });
            
            toast.error(`🤖 AI Agent 처리 실패: ${error.message}`, {
              duration: 5000,
              position: 'top-center',
            });
          } finally {
            console.log('✅ AI Agent 처리 완료, 모드 해제');
            setIsLoading(false);
            isProcessingRef.current = false;
            setSelectedAgentType(null); // 전송 후 모드 해제
          }
          return;
        }
        ...
        if (onStartChat) {
          console.log("🔄 ChatInput: onStartChat 경로 사용", { messageText, filesCount: uploadedFiles.length });
          
          // 메시지가 비어있으면 전송하지 않음
          if (!messageText && uploadedFiles.length === 0) {
            console.warn("⚠️ 메시지와 파일이 모두 비어있어 전송하지 않습니다");
            isProcessingRef.current = false;
            return;
          }
              ...
              <div className="relative flex-1 flex items-center gap-2 shrink min-w-0">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    disabled={isLoading}
                    className="inline-flex items-center justify-center h-8 px-3 rounded-md transition-all"
                    style={{
                      backgroundColor: selectedAgentType === 'AI 보도' ? 'hsl(var(--accent-main-100))' : 'hsl(var(--bg-200))',
                      color: selectedAgentType === 'AI 보도' ? 'white' : 'hsl(var(--text-400))',
                      fontSize: '0.75rem',
                      fontWeight: '600',
                  ...
                  {/* AI 교열 */}
                  <button
                    onClick={() => handleAgentButton('AI 교열')}
                    disabled={isLoading}
                    className="inline-flex items-center justify-center h-8 px-3 rounded-md transition-all"
                    style={{
                      backgroundColor: selectedAgentType === 'AI 교열' ? 'hsl(var(--accent-main-100))' : 'hsl(var(--bg-200))',
                      color: selectedAgentType === 'AI 교열' ? 'white' : 'hsl(var(--text-400))',
                      fontSize: '0.75rem',
                      fontWeight: '600',
                      cursor: isLoading ? 'not-allowed' : 'pointer',
                      opacity: isLoading ? 0.5 : 1,
                  ...
                  {/* AI 퇴고 */}
                  <button
                    onClick={() => handleAgentButton('AI 퇴고')}
                    disabled={isLoading}
                    className="inline-flex items-center justify-center h-8 px-3 rounded-md transition-all"
                    style={{
                      backgroundColor: selectedAgentType === 'AI 퇴고' ? 'hsl(var(--accent-main-100))' : 'hsl(var(--bg-200))',
                      color: selectedAgentType === 'AI 퇴고' ? 'white' : 'hsl(var(--text-400))',
                      fontSize: '0.75rem',
                      fontWeight: '600',
                      cursor: isLoading ? 'not-allowed' : 'pointer',
                      opacity: isLoading ? 0.5 : 1,
                  ...
                  {/* AI 외신 */}
                  <button
                    onClick={() => handleAgentButton('AI 외신')}
                    disabled={isLoading}
                    className="inline-flex items-center justify-center h-8 px-3 rounded-md transition-all"
                    style={{
                      backgroundColor: selectedAgentType === 'AI 외신' ? 'hsl(var(--accent-main-100))' : 'hsl(var(--bg-200))',
                      color: selectedAgentType === 'AI 외신' ? 'white' : 'hsl(var(--text-400))',
                      fontSize: '0.75rem',
                      fontWeight: '600',
                      cursor: isLoading ? 'not-allowed' : 'pointer',
                      opacity: isLoading ? 0.5 : 1,
