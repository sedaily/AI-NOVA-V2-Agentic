...
export const useChat = (initialMessage, selectedEngine = "11") => {
  ...
  const handleWebSocketMessage = useCallback((data) => {
    ...
    // 백엔드에서 보내는 메시지 타입 처리
    if (data.type === 'ai_chunk' || data.type === 'chunk') {
      const content = data.chunk || data.content || '';
      setStreamingMessage(prev => {
        const updated = prev ? prev + content : content;
        return updated;
      });
    ...
    } else if (data.type === 'chat_end' || data.type === 'end') {
      handleStreamEnd();
  ...
  useEffect(() => {
    ...
    const initWebSocket = async () => {
      ...
      try {
        await connectWebSocket(selectedEngine);
        ...
        if (connected) {
          ...
          console.log("✅ WebSocket 메시지 핸들러 등록 완료");
        } else {
          console.warn("⚠️ WebSocket 연결 실패 - 모의 모드로 동작");
        }
      } catch (error) {
