...
class WebSocketService {
  ...
  sendMessage(
    ...
    return new Promise((resolve, reject) => {
      ...
      // localStorage에서 선택된 모델 가져오기 (파라미터가 없으면)
...
export const sendChatMessage = (
  message,
  engineType,
  conversationHistoryOrWebSearch = null,
  conversationId = null,
  idempotencyKey = null,
  selectedModel = null,
  webSearchEnabled = false
) => {
  // 호환성 처리: 3번째 인자가 boolean이면 webSearchEnabled로 처리
  let actualHistory = conversationHistoryOrWebSearch;
  let actualWebSearch = webSearchEnabled;

  if (typeof conversationHistoryOrWebSearch === 'boolean') {
    actualWebSearch = conversationHistoryOrWebSearch;
    actualHistory = null;
  }
