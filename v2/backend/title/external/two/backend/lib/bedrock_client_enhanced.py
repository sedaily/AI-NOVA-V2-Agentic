...
# Claude 모델 설정
DEFAULT_MODEL_ID = "anthropic.claude-opus-4-6-v1:0"  # Opus 4.6 기본값 (최신)
MAX_TOKENS = 4096
TEMPERATURE = 0.7   # 균형잡힌 창의성
TOP_P = 0.85
TOP_K = 30

# Anthropic 모델 ID → AWS Bedrock 모델 ID 매핑
BEDROCK_MODEL_MAP = {
    # Opus 4.6 - Latest & most capable (기본값)
    "opus-4-20250514": "anthropic.claude-opus-4-6-v1:0",
    # Opus 4.5 - Previous flagship model
...
def get_bedrock_model_id(model_id: str) -> str:
    """
    프론트엔드 모델 ID를 AWS Bedrock 모델 ID로 변환
    """
    if model_id in BEDROCK_MODEL_MAP:
        return BEDROCK_MODEL_MAP[model_id]
    # 이미 Bedrock 형식이면 그대로 반환
    if model_id and (model_id.startswith("us.anthropic.") or model_id.startswith("anthropic.")):
        return model_id
    # 기본 모델 반환
...
def stream_claude_response_enhanced(
    ...
    model_id: str = None  # 모델 ID (프론트엔드에서 선택)
) -> Iterator[str]:
    """
    ...
    """
    # 모델 ID 변환 (Anthropic → Bedrock)
    bedrock_model_id = get_bedrock_model_id(model_id) if model_id else DEFAULT_MODEL_ID
...
class BedrockClientEnhanced:
    ...
    def stream_bedrock(
        ...
        Args:
            model_id: 프론트엔드에서 선택한 모델 ID (Anthropic 형식)
        """
        try:
...
사용자의 질문: {user_message}"""
            ...
            logger.info(f"Engine: {engine_type}, Role: {user_role}, Model: {model_id}")
