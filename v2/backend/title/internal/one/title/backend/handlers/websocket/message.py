...
def handler(event, context):
    ...
    try:
        ...
        # 메시지 전송 액션
            ...
            selected_model = body.get('selectedModel', 'claude-opus-4-5-20251101')  # 선택된 모델
            ...
            for chunk in websocket_service.stream_response(
                ...
                if not web_search_sent and web_search_enabled:
                    ...
                    logger.info(f"🔍 웹검색 결과 확인: success={web_search_result.get('success') if web_search_result else None}, has_citations={bool(web_search_result.get('citations', [])) if web_search_result else False}")

                    if web_search_result:
                        logger.info(f"🔍 웹검색 결과 상세: {json.dumps(web_search_result, ensure_ascii=False, default=str)[:500]}")

                    if web_search_result and web_search_result.get('success'):
                        citations = web_search_result.get('citations', [])
                        # 최적화된 검색 쿼리 사용 (없으면 사용자 메시지 축약)
                        display_query = web_search_result.get('display_query', user_message[:30] + ('...' if len(user_message) > 30 else ''))
                        logger.info(f"🔍 Citations 타입: {type(citations)}, 개수: {len(citations) if citations else 0}, display_query: {display_query}")
                        if citations:
                            logger.info(f"🔍 첫번째 citation 샘플: {citations[0] if citations else 'N/A'}")
                            send_message_to_client(connection_id, {
                                'type': 'web_search_results',
                                'query': display_query,  # 최적화된 쿼리 표시
                                'citations': citations,
                                'total_results': len(citations),
                                'timestamp': datetime.utcnow().isoformat() + 'Z'
                            }, apigateway_client)
                            logger.info(f"🌐 웹검색 결과 전송 완료: {len(citations)} 출처, 쿼리: {display_query}")
                        else:
                            logger.warning("⚠️ Citations 비어있음")
                    else:
                        logger.warning(f"⚠️ 웹검색 결과 없음 또는 실패: {web_search_result}")
