...
export const useWebSocketChat = ({
  ...
  const ttftMeasured = useRef(false);
    ...
    switch (message.type) {
      ...
      case "ai_chunk":
        if (message.chunk && currentAssistantMessageId.current) {
          // TTFT 측정: 첫 번째 청크 수신 시점
          if (!ttftMeasured.current && sendTimeRef.current) {
            const clientTTFT = performance.now() - sendTimeRef.current;
            console.log(`[Client TTFT] ${clientTTFT.toFixed(0)}ms`);
            ttftMeasured.current = true;
          }
  ...
  const initWebSocket = useCallback(async () => {
    try {
      ...
      if (messageToSend && !hasProcessedInitial.current) {
        ...
        setTimeout(async () => {
          setIsLoading(true);
          // TTFT 측정: 초기 메시지 전송 시점 기록
          sendTimeRef.current = performance.now();
          ttftMeasured.current = false;
          try {
