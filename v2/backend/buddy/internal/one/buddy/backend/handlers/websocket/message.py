...
def handler(event, context):
    ...
    try:
        ...
        # 메시지 전송 액션
            ...
            selected_model = body.get('selectedModel')  # 프론트엔드 모델 ID
            web_search_enabled = body.get('webSearchEnabled', False)  # 웹 검색 활성화 여부 (기본값: False)

            logger.info(f"Processing message for {engine_type}, user: {user_id}, role: {user_role}, model: {selected_model}, webSearch: {web_search_enabled}")
            ...
            for chunk in websocket_service.stream_response(
                ...
                # 첫 청크 전에 웹검색 결과 전송 (regression 패턴)
                if not web_search_sent and web_search_enabled:
                    web_result = getattr(websocket_service, 'last_web_search_result', None)
                    logger.info(f"🔍 웹검색 결과 확인: success={web_result.get('success') if web_result else None}")
