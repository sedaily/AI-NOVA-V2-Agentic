...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"""
        ...
        # CoT 기반 체계적 프롬프트 구조
        system_prompt = f"""# AI 뉴스 어시스턴트 시스템 프롬프트
...
자신의 모델명이나 기술적 정보에 대해 질문받으면 구체적인 모델명은 밝히지 말고, "AI 뉴스 어시스턴트"라고만 답변하세요.
...
def stream_claude_response_enhanced(
    ...
    Args:
        selected_model: 프론트엔드 모델 ID (claude-opus-4-5-20251101 등)
    """
    # 모델 ID 매핑
    model_mapping = {
        ...
        'claude-opus-4-1-20250805': 'us.anthropic.claude-opus-4-20250514-v1:0'
    }
    
    # 모델 ID 결정
    model_id = CLAUDE_MODEL_ID  # 기본값
    if selected_model and selected_model in model_mapping:
        model_id = model_mapping[selected_model]
        logger.info(f"🎯 Model selected: {selected_model} -> {model_id}")
    else:
            ...
            for event in stream:
                ...
                if chunk:
                    ...
                    # 출력 토큰 추적 (message_delta에서)
                        ...
                        logger.info(f"Streaming completed - Total usage: {usage_info}")
                        # 마지막에 usage 정보 yield (캐시 추적용)
