...
import { ArrowUp, TrendingDown, Globe } from "lucide-react";
...
const ChatPage = ({
  ...
  // 🌐 웹검색 상태 (localStorage에서 pendingWebSearch 확인)
  const [webSearchEnabled, setWebSearchEnabled] = useState(() => {
    const pendingWebSearch = localStorage.getItem('pendingWebSearch') === 'true';
    if (pendingWebSearch) {
      console.log("🌐 localStorage에서 웹검색 상태 복원: true");
      localStorage.removeItem('pendingWebSearch'); // 사용 후 즉시 삭제
    }
    return pendingWebSearch;
  });
  ...
  const webSearchResultsRef = useRef(null); // 웹검색 결과 ref (타이밍 문제 해결용)
      ...
      switch (message.type) {
        ...
        case "web_search_results":
          ...
          if (message.sources && message.sources.length > 0) {
            ...
            // ref에 먼저 저장 (동기적으로 즉시 사용 가능)
            webSearchResultsRef.current = webSearchData;
            // state도 업데이트 (UI 리렌더링용)
            setWebSearchResults(webSearchData);
            console.log("🌐 웹검색 결과 ref에 저장됨:", webSearchData);
          }
          return;
        ...
        case "ai_start":
          ...
          // 현재 웹검색 결과를 캡처 (AI 메시지에 첨부) - ref 사용으로 타이밍 문제 해결
          const currentWebSearchResults = webSearchResultsRef.current;
          console.log("🔍 ai_start에서 웹검색 결과 캡처:", currentWebSearchResults);
          ...
          setMessages((prev) => {
            ...
            console.log("✅ AI 메시지 컨테이너 추가:", {
              ...
              webSearchResultsData: currentWebSearchResults,
          ...
          const sendInitialMessage = async (retryCount = 0) => {
            ...
            try {
              ...
              while (!sendSuccess && sendRetries < 3) {
                try {
                  console.log("🌐 초기 메시지 웹검색 상태:", webSearchEnabled);
                  await sendChatMessage(
                    ...
                    selectedModel,
                    webSearchEnabled  // 🌐 웹검색 상태 전달
                  );
                  sendSuccess = true;
                  console.log("✅ Initial message 전송 완료 (webSearch:", webSearchEnabled, ")");
            ...
            >
              <div className="flex flex-col gap-3.5 m-3.5">
                ...
                <div className="flex gap-2.5 w-full items-center">
                  ...
                  {/* Claude Model Selector - 오른쪽 위치 (send 버튼 옆) */}
                  <div className="relative shrink-0" ref={modelDropdownRef}>
                      ...
                      {/* Dropdown Menu - 위로 열림 (ChatPage는 화면 하단이라 잘림 방지) */}
                      {isModelDropdownOpen && (
                        <div
                          className="absolute bottom-full mb-2 right-0 z-50 rounded-xl border shadow-lg overflow-hidden"
                          style={{
                            minWidth: "224px",
                            backgroundColor: "hsl(var(--bg-000))",
                            borderColor: "hsl(var(--border-300)/30%)",
                            ...
                            {CLAUDE_MODELS.map((model) => (
                              ...
                              >
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="text-xs text-text-400 truncate">{model.desc}</div>
                                </div>
                                {selectedModel === model.id && (
                                  <svg className="w-4 h-4 text-accent-main-100 shrink-0" fill="currentColor" viewBox="0 0 20 20">
