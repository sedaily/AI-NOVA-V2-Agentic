...
def handler(event, context):
    ...
    try:
        ...
        # GET /conversations/{conversationId} - 상세 조회
            ...
            conv_id = path_params['conversationId']
            logger.info(f"[DEBUG] Getting conversation: {conv_id}")

            conversation = conversation_service.get_conversation(conv_id)

            logger.info(f"[DEBUG] Conversation result: {conversation is not None}")
            if conversation:
                result = conversation.to_dict()
                logger.info(f"[DEBUG] Returning conversation with {len(result.get('messages', []))} messages")
                return APIResponse.success(result)
            else:
                logger.warning(f"[DEBUG] Conversation not found: {conv_id}")
