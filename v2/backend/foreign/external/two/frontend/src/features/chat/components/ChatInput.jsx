    ...
    const handleSubmit = async (e) => {
      ...
      if ((message.trim() || uploadedFiles.length > 0) && !isLoading) {
        ...
        if (onStartChat) {
          ...
          // 그 다음 페이지 전환 (파일 내용이 포함된 메시지 전달, 모델 정보 포함)
          onStartChat(fullMessage, selectedModel);
        ...
        if (!onStartChat && isConnected) {
          setIsLoading(true);
          try {
            console.log(`${engineType} 엔진으로 메시지 전송:`, messageText, `모델: ${selectedModel}, 웹검색: ${webSearchEnabled}`);
            await sendChatMessage(messageText, engineType, null, null, null, selectedModel, webSearchEnabled);
                  ...
                  >
                    <span className="truncate max-w-20">
                      {selectedModel === 'claude-opus-4-5-20251101' ? 'Opus 4.5' :
                       ...
                       selectedModel === 'claude-opus-4-1-20250805' ? 'Opus 4.1' : 'Opus 4.5'}
                    </span>
                    <svg
