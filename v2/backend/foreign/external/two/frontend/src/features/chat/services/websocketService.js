...
class WebSocketService {
  ...
  sendMessage(
    ...
    return new Promise((resolve, reject) => {
      ...
      const modelToUse = selectedModel || localStorage.getItem('selectedModel') || 'opus-4-20250514';

      try {
        // 사용자 정보 가져오기
        const userInfo = JSON.parse(localStorage.getItem("userInfo") || "{}");
        // username(UUID)를 우선 사용 - conversationService와 동일하게 맞춤
        const userId =
