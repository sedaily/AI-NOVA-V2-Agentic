...
export const useWebSocketChat = ({
      ...
      if (messageToSend && !hasProcessedInitial.current) {
        ...
        setTimeout(async () => {
          ...
          try {
            ...
            await sendChatMessage(messageToSend, selectedEngine, [], currentConversationId, null, null, webSearchEnabled);
  ...
  const sendMessage = useCallback(async (messageText, conversationHistory) => {
    ...
    // localStorage에서 직접 웹검색 설정 읽기 (bodo 방식)
    const webSearchEnabled = localStorage.getItem('webSearchEnabled') === 'true';

    try {
      if (isConnected) {
        await sendChatMessage(messageText, selectedEngine, conversationHistory, currentConversationId, null, null, webSearchEnabled);
        return true;
      } else {
