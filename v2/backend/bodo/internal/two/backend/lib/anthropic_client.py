...
# 모델 설정
DEFAULT_MODEL_ID = "claude-opus-4-5-20251101"  # Claude 4.5 Opus - 기본 모델
MAX_TOKENS = 4096
TEMPERATURE = 0.7

# 지원 모델 목록
SUPPORTED_MODELS = {
    "claude-opus-4-5-20251101": "Claude 4.5 Opus - Complex tasks",
    "claude-sonnet-4-5-20250929": "Claude 4.5 Sonnet - Everyday tasks",
    "claude-haiku-4-5-20251001": "Claude 4.5 Haiku - Fast responses",
}
...
def stream_anthropic_response(
    ...
    Args:
        ...
        model_id: 사용할 모델 ID (없으면 기본 모델 사용)
    ...
    try:
        ...
        # 모델 ID 검증 및 설정
        selected_model = model_id if model_id in SUPPORTED_MODELS else DEFAULT_MODEL_ID
        logger.info(f"Selected model: {selected_model} (requested: {model_id})")
...
class AnthropicClient:
    ...
    def stream_response(
        ...
        try:
            ...
            logger.info(f"Streaming with Anthropic API, model: {model_id}")
