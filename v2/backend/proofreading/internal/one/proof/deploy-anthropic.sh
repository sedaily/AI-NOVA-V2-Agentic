...
if aws lambda get-function --function-name $FIRST_FUNC --region $AWS_REGION &>/dev/null; then
    ...
    # Secrets Manager 정책 생성 (Anthropic + Perplexity)
  ...
  "Statement": [
    {
      ...
      "Resource": [
        "arn:aws:secretsmanager:$AWS_REGION:*:secret:proof-v1*",
