...
const ChatInput = forwardRef(
  (
    { onSendMessage, onStartChat, onTitlesGenerated, engineType = "Basic", showModelSelector = false },
    ref
  ) => {
    ...
    // Claude 모델 선택 상태 (localStorage에서 검증 후 로드)
    const [selectedModel, setSelectedModel] = useState(() => {
      const storedModel = localStorage.getItem('selectedModel');
      if (storedModel && VALID_MODEL_IDS.includes(storedModel)) {
        return storedModel;
      }
      return 'opus-4-20250514';
    });
    ...
    const handleSubmit = async (e) => {
      ...
      if (message.trim() && !isLoading) {
        ...
        if (onStartChat) {
          ...
          console.log("🔀 ChatPage로 네비게이션 - 메시지:", fullMessage, "웹검색:", webSearchEnabled);
          console.log("📁 업로드된 파일:", uploadedFiles.length, "개");

          // 웹검색 상태를 localStorage에 저장 (ChatPage에서 사용)
          if (webSearchEnabled) {
            localStorage.setItem('pendingWebSearch', 'true');
          } else {
            localStorage.removeItem('pendingWebSearch');
          }
          ...
          setIsTyping(false);
          setUploadedFiles([]);
          setWebSearchEnabled(false); // 웹검색 상태도 초기화
          if (textareaRef.current) {
            textareaRef.current.style.height = "auto";
        ...
        if (!onStartChat && isConnected) {
          setIsLoading(true);
          try {
            console.log(`${engineType} 엔진으로 메시지 전송:`, messageText, webSearchEnabled ? "(웹검색 ON)" : "");
            await sendChatMessage(messageText, engineType, webSearchEnabled);
                ...
                <div className="relative shrink-0 model-dropdown">
                  ...
                  >
                    <span className="truncate max-w-20">
                      {selectedModel === 'opus-4-20250514' ? 'Opus 4.6' :
                       selectedModel === 'claude-opus-4-5-20251101' ? 'Opus 4.5' :
                       selectedModel === 'claude-sonnet-4-5-20250929' ? 'Sonnet 4.5' : 'Opus 4.6'}
                    </span>
                    <svg
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              if (model.id === 'claude-opus-4-5-20251101') {
                                toast('Opus 4.5는 현재 API 서버 과부하로 응답이 지연될 수 있습니다.', {
                                  icon: '⚠️',
                                  duration: 4000,
