...
# 넥서스 크레딧(NC) 계산 상수
# 1NC = 1KRW, 운영환율 1USD = 1,500 KRW
EXCHANGE_RATE = 1500  # 운영 환율 (4.2% 버퍼 마진 포함)

# 모델별 USD 가격 (per 1M tokens)
MODEL_PRICING = {
    "claude-opus-4-5-20251101": {"input": 5.0, "output": 25.0},
    "claude-opus-4-1-20250805": {"input": 5.0, "output": 25.0},
    "claude-sonnet-4-5-20250929": {"input": 3.0, "output": 15.0},
}

# 모델별 1,000토큰당 크레딧 (NC)
# 공식: USD Price * 1.5 (= USD * 0.001 * 1500)
CREDIT_RATES = {
...
def calculate_credits(usage: Dict[str, int], model_id: str = None) -> Dict[str, float]:
    """
    넥서스 크레딧(NC) 계산

    Args:
        usage: 토큰 사용량 {'input_tokens': N, 'output_tokens': N, ...}
        model_id: 모델 ID (기본값: Opus 4.5)

    Returns:
        {
            'input_credits': 입력 크레딧,
            'output_credits': 출력 크레딧,
            'total_credits': 총 크레딧,
            'model_id': 사용된 모델 ID
        }
    """
    # 기본 모델: Opus 4.6
    if not model_id or model_id not in CREDIT_RATES:
        model_id = "opus-4-20250514"

    rates = CREDIT_RATES[model_id]
    ...
    return {
        'input_credits': round(input_credits, 2),
        'output_credits': round(output_credits, 2),
...
def stream_anthropic_response(
    ...
    Args:
        ...
        model_id: 사용할 모델 ID (없으면 기본 MODEL_ID 사용)
    ...
    try:
        ...
        logger.info(f"Calling Anthropic API with model: {selected_model}, caching: {enable_caching}")
        ...
        # 마지막에 usage_info를 특별한 마커와 함께 전달 (호출자가 캡처)
        yield f"__USAGE_INFO__:{json.dumps(usage_info)}"
...
class AnthropicClient:
    ...
    def stream_response_with_history(
        ...
        # 모델 선택 (파라미터 우선, 없으면 인스턴스 기본값)
        use_model = model_id or self.model_id
        logger.info(f"🚀 stream_response_with_history - model: {use_model}")
        ...
        try:
            ...
            static_system_prompt = replace_template_variables(system_prompt)

            # 대화 히스토리에 캐시 브레이크포인트 적용
            if enable_caching and len(messages) >= 2:
                ...
                logger.info(f"📤 Calling Anthropic API (no conversation cache), model: {use_model}, messages: {len(messages)}")
    ...
    def stream_response(
        ...
        try:
            ...
            logger.info(f"Streaming with Anthropic API (model: {selected_model})")
            ...
            for chunk in stream_anthropic_response(
                ...
                # usage_info 마커 캡처
                if chunk.startswith("__USAGE_INFO__:"):
                    try:
                        self.last_usage = json.loads(chunk[15:])
                        logger.info(f"📊 Captured usage: input={self.last_usage.get('input_tokens', 0)}, "
                                   f"output={self.last_usage.get('output_tokens', 0)}")
                    except json.JSONDecodeError:
                        logger.warning("Failed to parse usage info")
                else:
                    yield chunk
