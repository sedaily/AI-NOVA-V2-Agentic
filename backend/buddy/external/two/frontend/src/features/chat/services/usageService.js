...
const CREDITS_CACHE_DURATION = 10000;
  ...
  try {
    ...
    if (!forceRefresh) {
      ...
      if (cachedCredits && cachedTime) {
        if (Date.now() - parseInt(cachedTime) < CREDITS_CACHE_DURATION) {
    ...
    const cachedCredits = localStorage.getItem(CREDITS_CACHE_KEY);
    if (cachedCredits) return JSON.parse(cachedCredits);
    return { balance: 50000, initialCredit: 50000, totalUsed: 0 };
  } catch (error) {
...
export const updateCreditsCache = (balance, totalUsed) => {
  try {
    const cachedCredits = localStorage.getItem(CREDITS_CACHE_KEY);
    let creditData = cachedCredits ? JSON.parse(cachedCredits) : { initialCredit: 50000 };
...
export const formatCredits = (amount) => {
  ...
  return amount.toLocaleString('ko-KR', { minimumFractionDigits: 0, maximumFractionDigits: 1 });
};
