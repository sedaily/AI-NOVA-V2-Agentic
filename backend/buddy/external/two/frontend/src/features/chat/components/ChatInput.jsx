...
const ChatInput = forwardRef(
  (
    { onSendMessage, onStartChat, onTitlesGenerated, engineType = "11", showModelSelector = false, selectedModel, onModelChange },
    ref
  ) => {
    ...
    const handleSubmit = async (e) => {
      ...
      if ((message.trim() || uploadedFiles.length > 0) && !isLoading) {
        ...
        if (!onStartChat && isConnected) {
          setIsLoading(true);
          try {
            console.log(`${engineType} 엔진으로 메시지 전송:`, messageText, `웹검색: ${webSearchEnabled}`);
            await sendChatMessage(messageText, engineType, null, null, null, null, webSearchEnabled);
        ...
        >
          <div className="flex flex-col gap-3.5 m-3.5">
            ...
            <div className="flex gap-2.5 w-full items-center">
              ...
              {/* Claude Model Selector - 외신과 동일하게 flex-1 컨테이너 밖에 배치 */}
              {showModelSelector && (
