...
class WebSocketService {
  ...
  sendMessage(message, engineType = "11", conversationId = null, conversationHistory = null, idempotencyKey = null, selectedModel = 'claude-opus-4-5-20251101', webSearchEnabled = false) {
    return new Promise((resolve, reject) => {
      // WebSocket이 연결되지 않은 경우에만 모의 응답
      if (!this.isWebSocketConnected()) {
        console.log("⚠️ WebSocket 미연결: 모의 AI 응답 생성");
      ...
      try {
        const payload = {
          ...
          userId: localStorage.getItem('userInfo') ? JSON.parse(localStorage.getItem('userInfo')).username : (import.meta.env.VITE_CMS_USER_ID || 'anonymous'),
