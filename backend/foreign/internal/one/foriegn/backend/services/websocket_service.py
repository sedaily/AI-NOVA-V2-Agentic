...
from lib.perplexity_client import PerplexityClient, get_perplexity_client
...
class WebSocketService:
    ...
    def __init__(self):
        ...
        self.ai_client = self.bedrock_client
        ...
        # Perplexity 클라이언트 (웹 검색용)
        self.perplexity_client = get_perplexity_client()
        self.last_web_search_result = None  # 웹검색 결과 저장용
    ...
    def stream_response(
        ...
        """
        Bedrock 스트리밍 응답 생성 (Perplexity 웹검색 통합)
        ...
        try:
            ...
            # 웹검색 수행 (활성화된 경우)
            if web_search_enabled and self.perplexity_client and self.perplexity_client.is_available():
                logger.info(f"🔍 웹검색 시작: {user_message[:50]}...")
                try:
                    search_result = self.perplexity_client.search_for_context(
                    ...
                    if search_result.get('success'):
                        ...
                        web_search_context = self.perplexity_client.format_search_results_for_prompt(search_result)
                        logger.info(f"✅ 웹검색 완료: {len(search_result.get('citations', []))} 출처")
                    else:
            ...
            if web_search_context:
                enhanced_user_message = f"{web_search_context}\n\n위 검색 결과를 참고하여 다음 질문에 답변해주세요:\n\n{user_message}"
            ...
            if self.ai_provider == 'anthropic' and hasattr(self.ai_client, 'stream_response'):
                try:
                    ...
                    for chunk in self.ai_client.stream_response(
                        user_message=enhanced_user_message,  # 웹검색 컨텍스트 포함
