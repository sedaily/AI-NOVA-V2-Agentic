...
class ConversationRepository:
    ...
    def find_by_id(self, conversation_id: str, user_id: str = None) -> Optional[Conversation]:
        """ID로 대화 조회"""
        try:
            logger.info(f"find_by_id called: conversation_id={conversation_id}, user_id={user_id}")

            # user_id가 제공되지 않은 경우 조회 (페이지네이션 지원)
            if not user_id:
                # conversation_id로 스캔해서 실제 데이터를 찾음
                ...
                while True:
                    ...
                    if response.get('Items'):
                        logger.info(f"Found conversation after scanning {total_scanned} items")
                ...
                logger.warning(f"Conversation not found after scanning {total_scanned} items: {conversation_id}")
                return None
    ...
    def update_messages(self, conversation_id: str, messages: List[Message]) -> bool:
        ...
        try:
            ...
            messages_data = [
                {
                    'role': msg.role,
                    'type': msg.type or msg.role,  # type 필드 추가 (프론트엔드 호환성)
                    'content': msg.content,
