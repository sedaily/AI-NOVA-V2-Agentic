...
class ClaudeService {
  ...
  async streamChat(message, selectedModel, onChunk, onComplete, onError) {
    if (!this.hasApiKey()) {
      onError(new Error('Claude API 키가 설정되지 않았습니다. .env 파일에 VITE_CLAUDE_API_KEY를 설정해주세요.'));
      return;
    }
    ...
    try {
      ...
      const apiUrl = import.meta.env.DEV 
        ...
        : `${import.meta.env.VITE_API_BASE_URL}/api/claude/chat`;
      ...
      const response = await fetch(apiUrl, {
        ...
        body: JSON.stringify({
          message: message,
          model: selectedModel || 'claude-opus-4-20250514',
          apiKey: this.apiKey
        })
      });
      ...
      // 텍스트를 청크로 분할하여 스트리밍 효과
      const words = text.split('');
      let chunkIndex = 0;
      
      for (const char of words) {
        onChunk(char, chunkIndex);
        chunkIndex++;
