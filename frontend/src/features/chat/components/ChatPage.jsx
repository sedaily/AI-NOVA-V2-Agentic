...
// import AgentTestPanel from "./AgentTestPanel"; // 사용하지 않음
...
const ChatPage = ({
  ...
  // Agent 스트리밍 확인
  const [initialAgentStreaming] = useState(() => {
    const pendingStreaming = localStorage.getItem('pendingAgentStreaming');
    if (pendingStreaming) {
      try {
        const data = JSON.parse(pendingStreaming);
        localStorage.removeItem('pendingAgentStreaming');
        return data;
      } catch (error) {
        localStorage.removeItem('pendingAgentStreaming');
        return null;
    ...
    return location.state?.agentStreaming ? { agentType: 'AI', message: initialMessage } : null;
  });

  // Agent 응답 확인
  const [initialAgentData] = useState(() => {
    const pendingAgentResponse = localStorage.getItem('pendingAgentResponse');
    console.log("🔍 pendingAgentResponse 확인:", pendingAgentResponse);
    if (pendingAgentResponse) {
      try {
        const agentData = JSON.parse(pendingAgentResponse);
        console.log("📦 localStorage에서 Agent 응답 복원:", agentData);
        localStorage.removeItem('pendingAgentResponse'); // 사용 후 즉시 삭제
        return agentData;
      } catch (error) {
        console.error("Agent 데이터 파싱 실패:", error);
        localStorage.removeItem('pendingAgentResponse');
        return null;
      }
    }
    return location.state?.agentData || null;
  });
  ...
  const [messages, setMessages] = useState(() => {
    ...
    console.log("🎯 initialAgentData:", initialAgentData);
    console.log("🎯 URL conversationId:", urlConversationId);

    let hasCachedData = false;

    // Agent 응답이 있는 경우 처리
    if (initialAgentData && initialAgentData.agentResponse) {
      console.log("🤖 Agent 응답 초기 메시지 생성:", initialAgentData);
      
      const agentMessage = {
        id: crypto.randomUUID(),
        type: "assistant",
        content: `🤖 ${initialAgentData.agentType} 결과:\n\n${initialAgentData.agentResponse}`,
        timestamp: new Date(),
        isStreaming: false,
      };
    ...
    <div className="flex h-screen" 
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        ...
        <div className={clsx(
          ...
          {/* Agent Test Panel */}
          {/* <AgentTestPanel currentMessage={currentMessage} /> */}
        </div>
      </div>
