...
const ChatPage = ({
  ...
  // 🔑 현재 세션에서 생성된 대화 ID 추적 (race condition 방지)
  const locallyCreatedConversationRef = useRef(new Set());

  // 🔑 초기 대화 ID를 로컬 생성 목록에 추가 (메인 페이지에서 메시지와 함께 들어온 경우)
  useEffect(() => {
    // initialMessage가 있다는 것은 새 대화를 시작한다는 의미
    if (initialMessage && currentConversationId) {
      locallyCreatedConversationRef.current.add(currentConversationId);
      console.log("🔐 초기 메시지 감지 - 로컬 생성 목록에 추가:", currentConversationId);
    }
  }, []); // 컴포넌트 마운트 시 한 번만 실행
  ...
  useEffect(() => {
    ...
    if (loadConversationId && loadedConversationRef.current !== loadConversationId) {
        // 🔑 Race condition 방지: 로컬에서 방금 생성된 대화는 서버 로드 건너뛰기
        const isLocallyCreated = locallyCreatedConversationRef.current.has(loadConversationId);
        const hasLocalMessages = messages.length > 0;

        if (isLocallyCreated && hasLocalMessages) {
          console.log("🔒 Race condition 방지: 로컬에서 생성된 대화 - 서버 로드 건너뛰기:", loadConversationId);
          loadedConversationRef.current = loadConversationId;
        ...
        setTimeout(() => {
          ...
          .then((response) => {
            ...
            // 응답 null 체크 추가
            if (!response) {
              console.warn("⚠️ 서버 응답이 null입니다 - 메시지 초기화");
              setMessages([]);  // 대화를 찾을 수 없으면 빈 상태로 초기화
              loadedConversationRef.current = loadConversationId;  // 로드 시도 완료 표시
              return;
            }
            ...
            if (conversationData && conversationData.messages) {
              ...
              console.warn("⚠️ messages가 없음 - 메시지 초기화. 전체 구조:", {
                response,
                conversationData,
                hasConversation: !!response.conversation,
                hasMessages: !!(conversationData && conversationData.messages)
              });
              setMessages([]);  // 메시지가 없으면 빈 상태로 초기화
              loadedConversationRef.current = loadConversationId;  // 로드 시도 완료 표시
            }
          })
          .catch((error) => {
            console.error("서버에서 대화 불러오기 실패 - 메시지 초기화:", error);
            setMessages([]);  // 에러 시 빈 상태로 초기화
            loadedConversationRef.current = loadConversationId;  // 로드 시도 완료 표시
          })
          .finally(() => {
            setIsLoadingConversation(false);
      ...
      // 🔑 Race condition 방지: 로컬에서 생성된 대화 추적
      locallyCreatedConversationRef.current.add(newConversationId);
      console.log("🆕 새 대화 생성 - 로컬 생성 목록에 추가:", newConversationId);
  ...
  const webSearchResultsRef = useRef(null); // 동기적 접근용 ref

  // 모델 선택 상태 (localStorage 검증 포함)
  const [selectedModel, setSelectedModel] = useState(() => {
    ...
    // 유효하지 않으면 기본값으로 리셋
    const defaultModel = "opus-4-20250514";
    localStorage.setItem("selectedModel", defaultModel);
    console.log("🔄 Invalid model ID detected, reset to default:", defaultModel);
    return defaultModel;
  });
    ...
    const handleWebSocketMessage = (message) => {
      ...
      switch (message.type) {
        ...
        case "web_search_results":
          ...
          webSearchResultsRef.current = searchData;
          setWebSearchResults(searchData);
          return;
        ...
        case "chat_end":
          ...
          if (currentAssistantMessageId.current) {
            ...
            // 🌐 웹검색 결과를 메시지에 저장 (ref에서 가져옴)
            const savedWebSearchResults = webSearchResultsRef.current;
              ...
              const updated = prev.map((msg) =>
                msg.id === currentAssistantMessageId.current
                  ? {
                      ...
                      isStreaming: false,
                      webSearchResults: savedWebSearchResults, // 웹검색 결과 저장
                    }
                  : msg
              );
                  ...
                  return (
                    ...
                    >
                      {message.type === "user" ? (
                        ...
                        <>
                          {/* 웹검색 결과 표시 (스트리밍 중) */}
                          {webSearchResults && webSearchResults.sources && webSearchResults.sources.length > 0 && (
                            <WebSearchSources
                        ...
                        <>
                          {/* 웹검색 결과 표시 (완료된 메시지) */}
                          {message.webSearchResults && message.webSearchResults.sources && message.webSearchResults.sources.length > 0 && (
                            <WebSearchSources
                ...
                <div className="flex gap-2.5 w-full items-center">
                  <div className="relative flex-1 flex items-center gap-2 shrink min-w-0">
                    ...
                    <div className="relative shrink-0" ref={modelDropdownRef}>
                      ...
                      >
                        <span>
                          {CLAUDE_MODELS.find(m => m.id === selectedModel)?.name || "모델 선택"}
                        </span>
                        <ChevronDown
                          size={14}
                          className={clsx(
                  ...
                  <button
                    ...
                    title={webSearchEnabled ? "웹검색 활성화됨" : "웹검색 비활성화됨"}
                  >
                    <Globe size={16} />
                  </button>
                  ...
                  <div style={{ opacity: 1, transform: "none" }}>
                    <button
                      className={clsx(
                        ...
                        (isTyping || uploadedFiles.length > 0) && (creditBalance === null || creditBalance > 0)
                          ? "bg-accent-main-000 text-oncolor-100 hover:bg-accent-main-200"
                          : "bg-gray-600 text-gray-400 cursor-not-allowed"
                      )}
                      disabled={(!isTyping && uploadedFiles.length === 0) || (creditBalance !== null && creditBalance <= 0)}
                      type="button"
