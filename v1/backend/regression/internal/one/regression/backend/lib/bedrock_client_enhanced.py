...
def _map_model_id(frontend_model_id: str) -> str:
    """
    프론트엔드 모델 ID를 Bedrock 모델 ID로 매핑
    
    Args:
        frontend_model_id: 프론트엔드에서 전달된 모델 ID
    ...
    model_mapping = {
        ...
        'claude-opus-4-5-20251101': 'us.anthropic.claude-opus-4-20250514-v1:0',
        'claude-sonnet-4-5-20250929': 'us.anthropic.claude-sonnet-4-20250514-v1:0',
    }
    
    bedrock_model_id = model_mapping.get(frontend_model_id, 'us.anthropic.claude-opus-4-1-20250805-v1:0')
    logger.info(f"Model mapping: {frontend_model_id} → {bedrock_model_id}")
...
def stream_claude_response_enhanced(
    ...
    try:
        # 프론트엔드 모델 ID를 Bedrock 모델 ID로 매핑
        bedrock_model_id = _map_model_id(selected_model)
        ...
        if enable_caching and prompt_data:
            ...
            body = {
                ...
                "messages": messages,
                # "top_p": TOP_P,  # ❌ Claude Opus 4.6은 temperature와 함께 사용 불가
                "top_k": TOP_K
            }
        ...
        response = bedrock_runtime.invoke_model_with_response_stream(
            modelId=bedrock_model_id,  # 매핑된 Bedrock 모델 ID 사용
                    ...
                    if chunk_obj.get('type') == 'message_start':
                        ...
                        if usage:
                            ...
                            logger.info(f"📊 Cache metrics - "
                                      f"read: {usage_info['cache_read_input_tokens']}, "
                                      f"write: {usage_info['cache_creation_input_tokens']}, "
...
class BedrockClientEnhanced:
    ...
    def stream_response(
        ...
        """
        간단한 스트리밍 응답 생성 (모델 선택 지원)
        
        Args:
            user_message: 사용자 메시지
            system_prompt: 시스템 프롬프트
            conversation_context: 대화 컨텍스트
            model_id: 프론트엔드에서 선택한 모델 ID
        ...
        try:
            ...
            for chunk in stream_claude_response_enhanced(
                ...
                enable_caching=False  # 간단한 버전은 캐싱 비활성화
            ):
                yield chunk
        except Exception as e:
    ...
    def stream_bedrock(
        ...
        try:
            ...
            for chunk in stream_claude_response_enhanced(
                ...
                selected_model=selected_model,  # 프론트엔드에서 선택한 모델
