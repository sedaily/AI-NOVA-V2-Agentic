...
def handler(event, context):
    ...
    try:
        ...
        # 메시지 전송 액션
            ...
            # 모델 ID 추출 (camelCase와 snake_case 모두 지원)
            model_id = body.get('modelId') or body.get('model_id') or 'claude-opus-4-5-20251101'
            web_search_enabled = body.get('webSearchEnabled', False)  # 웹검색 활성화 여부 (기본값: False)

            logger.info(f"Processing message for {engine_type}, user: {user_id}, role: {user_role}, model: {model_id}, web_search: {web_search_enabled}")
