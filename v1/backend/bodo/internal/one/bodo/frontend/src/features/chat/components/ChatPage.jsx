...
const ChatPage = ({
  ...
  const [webSearchStatus, setWebSearchStatus] = useState(null); // 검색 상태 메시지
  const [webSearchResults, setWebSearchResults] = useState(null); // 검색 결과
  const [searchMessageIndex, setSearchMessageIndex] = useState(0); // 검색 메시지 인덱스
  const webSearchResultsRef = useRef(null); // 웹검색 결과 UI 참조
      ...
      switch (message.type) {
        ...
        case "web_search_results":
          ...
          console.log("🌐 웹검색 결과 수신:", {
            query: message.query,
            totalResults: message.total_results,
        ...
        case "chat_end":
          ...
          if (currentAssistantMessageId.current) {
            ...
            // 웹검색 결과를 ref에 저장 (closure 문제 해결)
            const currentWebSearchResults = webSearchResults;
            webSearchResultsRef.current = currentWebSearchResults;
                          ...
                          webSearchResultsUI={
                            webSearchResults ? (
                              <WebSearchSources
                                ...
                                sources={webSearchResults.citations}
                              />
                            ) : null
                          }
                        />
                          ...
                          webSearchResultsUI={
                            message.webSearchResults ? (
                              <WebSearchSources
                                ...
                                sources={message.webSearchResults.citations}
                              />
                            ) : null
                          }
                        />
