      ...
      try {
        ...
        this.ws.onmessage = (event) => {
          try {
            ...
            // TTFT 측정: 첫 번째 ai_chunk 수신 시점
            if (data.type === 'ai_chunk' && !this.ttftMeasured && this.sendTime) {
              const clientTTFT = performance.now() - this.sendTime;
        ...
        if (messageChunks.length > 1) {
          ...
          messageChunks.forEach((chunk, index) => {
            const payload = {
              ...
              selectedModel: selectedModel, // 선택된 모델
