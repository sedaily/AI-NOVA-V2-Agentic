...
// 크레딧 API 엔드포인트 (Usage API와 동일한 베이스 사용)
const CREDITS_API_BASE_URL = USAGE_API_BASE_URL;

// 로컬 크레딧 캐시 키
const CREDITS_CACHE_KEY = 'user_credits_cache';
const CREDITS_CACHE_TIME_KEY = 'user_credits_cache_time';
const CREDITS_CACHE_DURATION = 10000; // 10초 캐시

/**
 * 사용자 크레딧 잔액 조회
 * - 서버에서 잔액 조회
 * - 없으면 자동으로 50,000 NC 생성
 *
 * @param {boolean} forceRefresh - 캐시 무시하고 강제 새로고침
 * @returns {Promise<{balance: number, initialCredit: number, totalUsed: number}>}
 */
export const getUserCredits = async (forceRefresh = false) => {
  try {
    const user = getCurrentUser();
    ...
    if (!forceRefresh) {
      ...
      const cachedTime = localStorage.getItem(CREDITS_CACHE_TIME_KEY);

      if (cachedCredits && cachedTime) {
        const timeDiff = Date.now() - parseInt(cachedTime);
        if (timeDiff < CREDITS_CACHE_DURATION) {
          console.log('💳 캐시된 크레딧 반환');
    ...
    // 서버에서 크레딧 조회
    const response = await fetch(
      `${CREDITS_API_BASE_URL}/credits/${encodeURIComponent(user.userId)}`,
      {
        method: 'GET',
        headers: getAuthHeaders(),
      }
    ...
    if (response.ok) {
      ...
      if (result.success) {
        const creditData = {
          ...
          initialCredit: result.initialCredit || 50000,
        ...
        // 캐시 저장
        localStorage.setItem(CREDITS_CACHE_KEY, JSON.stringify(creditData));
        localStorage.setItem(CREDITS_CACHE_TIME_KEY, Date.now().toString());

        console.log(`💳 크레딧 조회 성공: ${creditData.balance.toFixed(1)} NC`);
        return creditData;
      }
    }
    ...
    const cachedCredits = localStorage.getItem(CREDITS_CACHE_KEY);
    if (cachedCredits) {
      console.log('⚠️ 서버 오류, 캐시된 크레딧 사용');
      return JSON.parse(cachedCredits);
    }
...
/**
 * 크레딧 잔액 캐시 업데이트
 * chat_end 메시지에서 받은 잔액 정보로 캐시 업데이트
 *
 * @param {number} balance - 새 잔액
 * @param {number} totalUsed - 총 사용량
 */
export const updateCreditsCache = (balance, totalUsed) => {
  try {
    const cachedCredits = localStorage.getItem(CREDITS_CACHE_KEY);
    let creditData = cachedCredits ? JSON.parse(cachedCredits) : {
      initialCredit: 50000,
    };

    creditData.balance = balance;
    creditData.totalUsed = totalUsed;
    creditData.updatedAt = new Date().toISOString();

    localStorage.setItem(CREDITS_CACHE_KEY, JSON.stringify(creditData));
    localStorage.setItem(CREDITS_CACHE_TIME_KEY, Date.now().toString());

    console.log(`💳 크레딧 캐시 업데이트: ${balance.toFixed(1)} NC`);
  } catch (error) {
    console.error('크레딧 캐시 업데이트 실패:', error);
  }
};
 ...
 * 크레딧 잔액 캐시 정리
 */
export const clearCreditsCache = () => {
  localStorage.removeItem(CREDITS_CACHE_KEY);
  localStorage.removeItem(CREDITS_CACHE_TIME_KEY);
  console.log('🗑️ 크레딧 캐시 정리 완료');
};

/**
 * 크레딧 잔액 포맷팅 (천 단위 콤마)
 * @param {number} amount - 크레딧 금액
