...
const Header = ({
  ...
  usagePercentage,
  isLandingPage = false,
  creditBalance = null, // 부모 컴포넌트에서 전달받은 잔액
}) => {
  ...
  useEffect(() => {
    const loadCredits = async () => {
      ...
      // 부모에서 전달받은 잔액이 있으면 사용
      if (creditBalance !== null) {
        setCredits({ balance: creditBalance });
        return;
      }
    ...
    const handleCreditsUpdate = (event) => {
      if (event.detail && event.detail.balance !== undefined) {
        setCredits({ balance: event.detail.balance, totalUsed: event.detail.totalUsed });
      }
    };
  ...
  return (
    <header
      className={`sticky top-0 z-50 w-full transition-all duration-200 ${isLandingPage ? 'border-b border-white/10' : ''}`}
      style={{
        backdropFilter: "blur(20px)",
        backgroundColor: isLandingPage ? "rgba(12, 12, 14, 0.92)" : "hsl(var(--bg-100)/0.95)",
      }}
    >
        ...
        <div className="flex items-center justify-between h-16 relative">
          ...
          <div className="ml-auto flex items-center space-x-4">
            ...
            {isLoggedIn && credits && !isLandingPage && (
              <div
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-bg-200/80 border border-border-300/20 cursor-default"
                title={`총 사용: ${formatCredits(credits.totalUsed || 0)} NC`}
              >
